...
//     $conn = oci_connect('system', '123456', '47.95.212.18/ORCL', "AL32UTF8");
...
$sql_ins = "CREATE OR REPLACE PROCEDURE addOverhead 
(v_overhead_id IN overhead.overhead_id%TYPE,
v_overhead_type IN overhead.overhead_type%TYPE,
v_overhead_price IN overhead.overhead_price%TYPE,
v_overhead_date IN overhead.overhead_date%TYPE
) 
IS 
BEGIN 
INSERT INTO SCOTT.overhead values(v_overhead_id,v_overhead_type,v_overhead_price,v_overhead_date,'nothing',1);
END addOverhead; 
";
$statement = oci_parse($conn, $sql_ins);
if (oci_execute($statement)) {
...
$sql_create_trigger = "create or replace trigger addInventory_updateGoods
    after insert on scott.inventory
    for each row
    begin
      update scott.goods set quantity=:new.quantity+quantity
      where goods_id=:new.goods_id;
    end;";
$statement = oci_parse($conn, $sql_create_trigger);
if (oci_execute($statement)) {
    echo "y";
} else
    echo "n";
